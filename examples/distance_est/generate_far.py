#!/usr/bin/env python3
"""Generate FAR-band sequences (~100–200 m) outside frozen DZ / held mid.
...
Usage:
  python examples/distance_est/generate_far.py
...
FAR_NEAR_M_LO, FAR_NEAR_M_HI = 100.0, 150.0
FAR_OUTER_M_LO, FAR_OUTER_M_HI = 150.0, 200.0
FAR_M_LO, FAR_M_HI = 100.0, 200.0
...
OUT_DEFAULT = ROOT / "data" / "video_synth" / "distance_est" / "far"


def build_far_sequence(idx: int, n_frames: int, seed: int, width: int = 384, height: int = 192) -> DistSequence:
    ...
    if target == "near":
        lo, hi = FAR_NEAR_M_LO, FAR_NEAR_M_HI
        bld_lo, bld_hi = 90.0, 220.0
    else:
        lo, hi = FAR_OUTER_M_LO, FAR_OUTER_M_HI
        bld_lo, bld_hi = 120.0, 240.0
    ...
    # --- CRITICAL far-band objects with approach/recede ---
    ...
    for c in range(n_crit):
        ...
        if cls == "car":
            ...
            objs.append(WorldObj(
                ...
                name=f"intersection_far_{target}_{c+1}",
            ))
        elif cls == "stop_sign":
            ...
            objs.append(WorldObj(
                ...
                true_height_m=0.6, true_width_m=4.0,
                name=f"intersection_solo_far_{c}",
            ))

    # Distractors outside far band (near / DZ / mid) so scale + frozen DZ still present
    for i in range(2):
        d_dz = rng.uniform(35.0, 65.0)
        car_h = rng.uniform(1.4, 1.9)
        objs.append(WorldObj(
            oid=f"car_dz{i}", cls="car", color=CAR_COLORS[(i + 3) % len(CAR_COLORS)],
            d0_m=d_dz, v_depth=rng.choice([-0.5, 0.0, 0.5]),
    ...
    # One mid-outer light for band variety
    objs.append(WorldObj(
        oid="L_mid", cls="light", color="pole",
        d0_m=rng.uniform(70.0, 95.0), v_depth=0.0,
        lateral0_m=rng.uniform(-20, 20), v_lat=0.0,
        true_height_m=rng.uniform(3.0, 5.0), true_width_m=1.4,
        phase_offset=rng.randint(0, 19), name=f"{street} mid",
    ))

    return DistSequence(
        seq_id=f"{('fn' if target == 'near' else 'fo')}_{idx:03d}_{street.lower().replace(' ', '_')}",
...
def _obj_state(o: WorldObj, t: int, focal: float, W: int, H: int) -> dict[str, Any]:
    ...
    out = {
        ...
        "_in_far": in_far(d),
        "_in_far_near": in_far_near(d),
        "_in_far_outer": in_far_outer(d),
        "_v_depth": o.v_depth,
...
def generate(out_dir: Path, n_seq: int = 96, n_frames: int = 12, seed: int = 24092480, train_frac: float = 0.8) -> dict[str, Any]:
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    for p in list(out_dir.glob("fn_*")) + list(out_dir.glob("fo_*")):
        if p.is_dir():
            shutil.rmtree(p)
    ...
    for i in range(n_seq):
        ...
        seq = build_far_sequence(i, n_frames, seed)
        ...
        for t in range(seq.n_frames):
            ...
            for s in states:
                ...
                gt_objs.append({
                    ...
                    "in_far_near": s.get("_in_far_near"),
                    "in_far_outer": s.get("_in_far_outer"),
                ...
                if s.get("_in_far_near"):
                    near_obs += 1
                    if s["class"] in PRIORITY:
                        priority_near += 1
                if s.get("_in_far_outer"):
                    outer_obs += 1
                    if s["class"] in PRIORITY:
                        priority_outer += 1
                ...
                if ar == "approach":
                    ...
                    stable_n += 1
                in_tgt = (in_far_near(s["_gt_m"]) if target == "near" else in_far_outer(s["_gt_m"]))
        ...
        meta = {
            "domain": "distance_est",
            "family": "video_synth_temporal_far",
            "far_near_m": [FAR_NEAR_M_LO, FAR_NEAR_M_HI],
            "far_outer_m": [FAR_OUTER_M_LO, FAR_OUTER_M_HI],
            ...
            "note": "NO ground-truth meters here. GT in distances_gt.json sidecar only. "
                    f"Critical objects targeted to far_{target} with approach/recede. DZ frozen.",
        }
            ...
            "objects_t0": [
                {
                    ...
                    "in_far_near": in_far_near(o.d0_m),
                    "in_far_outer": in_far_outer(o.d0_m),
    ...
    summary = {
        ...
        "far_near_observations": near_obs,
        "far_outer_observations": outer_obs,
        "danger_zone_observations": dz_obs,
        "priority_far_near_observations": priority_near,
        "priority_far_outer_observations": priority_outer,
    ...
    lines = [
        "# FAR-band (100–150 / 150–200 m) focused set SUMMARY",
        "",
        f"- **mid_near:** {FAR_NEAR_M_LO}–{FAR_NEAR_M_HI} m · **mid_outer:** {FAR_OUTER_M_LO}–{FAR_OUTER_M_HI} m",
        ...
        f"- DZ 30–70 m frozen; mid floors held; soft priors car~4.5 / ped~1.7",
        "",
    ]
...
def main(argv=None):
    ...
    ap.add_argument("--seed", type=int, default=24092480)
    ap.add_argument("--train-frac", type=float, default=0.8)
    args = ap.parse_args(argv)
    print(f"Generating far → {args.out}", flush=True)
    ...
    print(json.dumps({
        ...
        "n_with_critical_far_t0": summary["n_sequences_with_critical_target_t0"],
        "split": summary["split"],
        "priority_far_near_observations": summary["priority_far_near_observations"],
